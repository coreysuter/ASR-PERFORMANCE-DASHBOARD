...
function brakeRowHtml(key,label,mappedCat){
  ...
  app.innerHTML = `
    <div class="panel goalsBig halfPage">
      <div class="goalsBigTop">
        <div class="goalsTitleRow" style="display:flex; align-items:center; gap:14px;">
          <div class="goalsTitleLeft" style="display:flex; align-items:center; gap:14px; min-width:260px;">
          ...
          <div class="goalsTitleMid" style="flex:1; display:flex; justify-content:center;">
            <div class="goalsMidBox" style="min-width:240px; max-width:340px; text-align:center; border:1px solid rgba(255,255,255,.18); border-radius:14px; padding:10px 14px;">
              <div style="font-size:12px; letter-spacing:.12em; opacity:.85; font-weight:800;">GOALS</div>
              <div style="display:flex; justify-content:center; gap:22px; margin-top:6px;">
                <div style="min-width:90px;">
                  <div id="goals_mid_asrro" style="font-size:22px; font-weight:900; line-height:1;">0.00</div>
                  <div style="font-size:12px; opacity:.75; margin-top:3px;">ASRs/RO</div>
                </div>
                <div style="min-width:90px;">
                  <div id="goals_mid_soldro" style="font-size:22px; font-weight:900; line-height:1;">0.00</div>
                  <div style="font-size:12px; opacity:.75; margin-top:3px;">SOLD</div>
                </div>
              </div>
            </div>
          </div>

          <div class="goalsTitleRight" style="min-width:260px;"></div>
        </div>
      </div>
  ...
  // -------------------- Live update for centered GOALS section --------------------
  // ASRs/RO = sum over services of (ASR% / 100)
  // SOLD (Sold/RO) = sum over services of (ASR%/100) * (Sold%/100)
  function _pctVal(v){
  ...
  function _setMid(id, val){
    const el = document.getElementById(id);
    if(el) el.textContent = Number(val || 0).toFixed(2);
  }
  function recomputeMidGoals(){
    let asrRo = 0;
    let soldRo = 0;

    // Regular goal rows (req/close)
    document.querySelectorAll('.goalRow input.goalMini[id$="_req"]').forEach(reqEl=>{
      const closeEl = document.getElementById(reqEl.id.replace(/_req$/, "_close"));
      if(!closeEl) return;
      const asrPct = _pctVal(reqEl.value);
      const soldPct = _pctVal(closeEl.value);
      const a = asrPct / 100;
      asrRo += a;
      soldRo += a * (soldPct / 100);
    });

    // Brakes (red fields)
    document.querySelectorAll('input[id^="b_"][id$="_req_red"]').forEach(reqEl=>{
      const closeEl = document.getElementById(reqEl.id.replace(/_req_red$/, "_close_red"));
      if(!closeEl) return;
      const asrPct = _pctVal(reqEl.value);
      const soldPct = _pctVal(closeEl.value);
      const a = asrPct / 100;
    ...
    // Tires (red fields)
    document.querySelectorAll('input[id^="t_"][id$="_req_red"]').forEach(reqEl=>{
      const closeEl = document.getElementById(reqEl.id.replace(/_req_red$/, "_close_red"));
      if(!closeEl) return;
      const asrPct = _pctVal(reqEl.value);
      const soldPct = _pctVal(closeEl.value);
    ...
    _setMid('goals_mid_asrro', asrRo);
    _setMid('goals_mid_soldro', soldRo);
  }

  // Initial compute + live updates
  requestAnimationFrame(recomputeMidGoals);
  const panel = document.querySelector('.panel.goalsBig');
  if(panel){
    panel.addEventListener('input', ()=>requestAnimationFrame(recomputeMidGoals), true);
    panel.addEventListener('change', ()=>requestAnimationFrame(recomputeMidGoals), true);
  }
