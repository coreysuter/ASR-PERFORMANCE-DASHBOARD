function renderServicesHome(){
  ...
  (function ensureServicesDashOverrides(){
    ...
    el.textContent = `
      ...
      .pageServicesDash .techHeaderPanel .mainFiltersBar .controls.mainAlwaysOpen{
        grid-template-columns:repeat(var(--cols,3), minmax(160px,1fr)) !important;
      }
  ...
  const teamKey = (st.team === 'express' || st.team === 'kia' || st.team === 'store') ? st.team : 'store';
  st.team = teamKey;

  let comparison = (st.comparison === 'team' || st.comparison === 'store' || st.comparison === 'goal') ? st.comparison : 'goal';
      ...
      <div class="phead">
        <div class="titleRow techTitleRow">
          ...
          <div class="techNameWrap">
            ...
            <div class="techTeamLine">Team: ${teamKey.toUpperCase()} <span class="teamDot">•</span> Comparison: ${comparison.toUpperCase()} <span class="teamDot">•</span> ${focus.toUpperCase()}</div>
          </div>
        ...
        <div class="mainFiltersBar">
          <div class="controls mainAlwaysOpen" style="--cols:${focus==='goal' ? 4 : 3}">
            <div>
              <label>Team</label>
              <select data-svcdash="1" data-ctl="team">
                ...
                <option value="store" ${teamKey==='store'?'selected':''}>Store</option>
              </select>
            </div>
  ...
  app.querySelectorAll('select[data-svcdash="1"]').forEach(sel=>{
    ...
    sel.addEventListener('change', ()=>{
      if(ctl==='focus'){
        st.focus = sel.value;
        if(sel.value==='goal') st.comparison = 'goal';
      }
