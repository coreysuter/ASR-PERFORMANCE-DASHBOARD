...
// read querystring from hash
  ...
  const techs = (() => {
    ...
    return getTechsByTeam(teamKey).map(_filteredEntity);
...
document.getElementById("app").innerHTML = header + `
  ...
  window.addEventListener('globalDateChange', function _groupDateHandler() {
    window.removeEventListener('globalDateChange', _groupDateHandler);
    if (typeof window.renderGroupPage === 'function') window.renderGroupPage(groupKey);
  });
}
