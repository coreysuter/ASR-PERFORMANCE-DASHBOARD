function renderServiceSettingsPage(){
  ...
  function _buildConfig(){
    return {
      ...
      services:    opcRows.map(r=>({
        service: r.service,
        opCodes: String(r.opcode||"").split(",").map(s=>s.trim()).filter(Boolean),
        descriptions: String(r.desc||"").split(/,\s+(?=[A-Z])/).map(s=>s.trim()).filter(Boolean),
        minMiles: map[String(r.service||"").trim()] || 0
      }))
    };
  }
  ...
  function _triggerSync(){
    ...
    _syncTimer = setTimeout(async ()=>{
      ...
      if(!token) return;
      const el = document.getElementById("svcSetSaved");
      if(el){ el.style.display="block"; el.textContent="↑ Syncing to GitHub…"; }
      const result = await window.commitToGitHub(_buildConfig());
      if(el){
        el.textContent = result.ok ? "✓ Saved to GitHub" : `✗ GitHub sync failed`;
        el.title = result.ok ? "" : (result.err||"");
        clearTimeout(_triggerSync._t);
        _triggerSync._t = setTimeout(()=>{ el.style.display="none"; el.title=""; }, 3000);
      }
    }, 1200);
  }
  ...
  function renderOpcTable(){
    ...
    function setItems(rowId, field, items){
      ...
      _saveOpc(opcRows); opcFlashSaved(); _triggerSync();
    }
    ...
    wrap.querySelectorAll(".opcPillText").forEach(el=>{
      el.addEventListener("blur",()=>{
        ...
        if(idxAttr===null){
          ...
          if(i>=0){ opcRows[i].service=el.textContent.trim(); _saveOpc(opcRows); opcFlashSaved(); _triggerSync(); }
          return;
        }
    ...
    wrap.querySelectorAll("[data-opc-del]").forEach(b=>{
      b.addEventListener("click",()=>{
        ...
        _saveOpc(opcRows); renderOpcTable(); opcFlashSaved(); _triggerSync();
      });
    });
  }
