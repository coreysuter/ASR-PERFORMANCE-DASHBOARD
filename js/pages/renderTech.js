...
const tfOpen = !!UI.techFilters[techId];
  ...
  // Tech name header: always wrap First on top, Last on bottom (header only)
  const __nameParts = String(t.name || "").trim().split(/\s+/).filter(Boolean);
  const __firstName = __nameParts.shift() || "";
  const __lastName = __nameParts.join(" ");
  const __techNameHTML = __lastName
    ? `<span style="display:block">${safe(__firstName)}</span><span style="display:block">${safe(__lastName)}</span>`
    : `${safe(__firstName)}`;
      ...
      <div class="phead">
        <div class="titleRow techTitleRow">
          ...
          <div class="techNameWrap">
            <div class="h2 techH2Big">${__techNameHTML}</div>
