      ...
      <div class="techRow pickRowFrame">
        <div class="techRowLeft">
          ...
          <button type="button" class="tbJump" data-cat="${safeSvcId(item.cat)}"
            style="background:transparent;border:none;padding:0;color:inherit;cursor:pointer;text-align:left;text-decoration:underline;text-underline-offset:2px">
            ${safe(item.label)}
          </button>
        </div>
        <div class="mini" style="font-size:18px;font-weight:800">${metricLbl} = ${fmtPct(metric)}</div>
      </div>
    `;
  }
