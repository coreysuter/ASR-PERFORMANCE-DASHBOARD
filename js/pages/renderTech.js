...
const hash = location.hash || "";
  ...
  const _cmpLabel = (compareBasis==="store") ? "Store" : "Team";

  // --- Popup data builders for each dial type ---
  ...
  // Sold % dial (tech vs team/store avg)
  function _popupSold(cmpClose, techClose, pctCmpClose){
    const gp = _gradeAndPct(pctCmpClose);
    return { rows:[
      { label: _cmpLabel+" Avg Sold %", value: _fmtPctPop(cmpClose) },
      { label: "Tech's Sold %", value: _fmtPctPop(techClose) }
    ], grade: gp.grade, pctAttained: gp.pctAttained };
  }

  // Sold Goal dial (tech vs goal)
  function _popupSoldGoal(goalClose, techClose, pctGoalClose){
    ...
    return { rows:[
      ...
      { label: "Tech's Sold/ASR %", value: _fmtPctPop(techClose) }
    ], grade: gp.grade, pctAttained: gp.pctAttained };
  }

  // ASR Goal dial (tech vs goal)
  function _popupAsrGoal(goalReq, techReq, pctGoalReq){
    ...
    return { rows:[
      ...
      { label: "Tech's ASRs/RO", value: _fmtDec(techReq) }
    ], grade: gp.grade, pctAttained: gp.pctAttained };
  }

  // ASRs/RO dial (tech vs team/store avg)
  function _popupAsr(cmpReq, techReq, pctCmpReq){
    const gp = _gradeAndPct(pctCmpReq);
    return { rows:[
      { label: _cmpLabel+" Avg ASRs/RO", value: _fmtDec(cmpReq) },
      { label: "Tech's ASRs/RO", value: _fmtDec(techReq) }
    ], grade: gp.grade, pctAttained: gp.pctAttained };
  }
