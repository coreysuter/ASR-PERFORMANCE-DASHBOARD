...
function renderAdvisorMain(){
  ...
  if(!advisors.length){
    app.innerHTML = `<div class="panel"><div class="phead"><div class="h2">Advisor Dashboard</div><div class="sub">No advisor data found (expected DATA.advisors).</div></div></div>`;
    return;
  }
  ...
  const soldAsrs = advisors.reduce((s,a)=>s+(Number(a.summary?.total?.sold)||0),0);
  const advisorSold = advisors.reduce((s,a)=>s+(Number(a.summary?.total?.advisor_sold)||0),0);
  const soldTotal = advisors.reduce((s,a)=>s+(Number(a.summary?.total?.sold_total)||0),0);

  const asrPerRo = totalRos ? (totalAsr/totalRos) : null;
  const soldRo = totalRos ? (soldTotal/totalRos) : null;
  const soldAsrsPct = totalAsr ? (soldAsrs/totalAsr) : null;

  function fmtIntLocal(v){ const n=Number(v); return Number.isFinite(n) ? Math.round(n).toLocaleString() : "—"; }
  function fmtNumLocal(v, d=1){ const n=Number(v); return Number.isFinite(n) ? n.toFixed(d) : "—"; }
  function fmtPctLocal(v){ const n=Number(v); return Number.isFinite(n) ? Math.round(n*100) + "%" : "—"; }

  // Sort: default by Sold/RO (total)
  const sorted = advisors.slice().sort((a,b)=>{
    const av = Number(a.summary?.total?.sold_ro);
    const bv = Number(b.summary?.total?.sold_ro);
  ...
  const header = `
    <div class="panel">
      <div class="phead">
        <div class="h2">Advisor Dashboard</div>
        <div class="sub">Advisor Sold (pre-inspection) + Sold ASRs = Total Sold. Sold/RO uses Total Sold.</div>
      </div>
      <div class="rowGrid" style="grid-template-columns: repeat(5, 1fr); margin-top:6px">
        <div class="cell"><span class="lbl">ROs</span><span class="val">${fmtIntLocal(totalRos)}</span></div>
        <div class="cell"><span class="lbl">Avg ODO</span><span class="val">${fmtIntLocal(avgOdo)}</span></div>
        <div class="cell"><span class="lbl">ASRs/RO</span><span class="val">${fmtNumLocal(asrPerRo,2)}</span></div>
        <div class="cell"><span class="lbl">Total Sold/RO</span><span class="val">${fmtPctLocal(soldRo)}</span></div>
        <div class="cell"><span class="lbl">Sold ASRs %</span><span class="val">${fmtPctLocal(soldAsrsPct)}</span></div>
      </div>
    </div>
  `;

  const rows = sorted.map(a=>{
    const ros = Number(a.ros)||0;
    const odo = Number(a.odo)||0;
    const asr = Number(a.summary?.total?.asr)||0;
    const asrPer = (ros? (asr/ros): null);

    const sold_asrs = Number(a.summary?.total?.sold)||0;
    const advisor_sold = Number(a.summary?.total?.advisor_sold)||0;
    const sold_total = Number(a.summary?.total?.sold_total)|| (sold_asrs+advisor_sold);
    const sold_ro = ros ? (sold_total/ros) : null;
    const sold_asrs_pct = asr ? (sold_asrs/asr) : null;
      ...
      <div class="techRow">
        <div class="rowGrid">
          <div class="cell">
            ...
            <span class="val" style="font-weight:800">${(a.name||"")}</span>
          </div>

          <div class="cell">
            <span class="lbl">ROs</span>
            <span class="val">${fmtIntLocal(ros)}</span>
          </div>

          <div class="cell">
            <span class="lbl">ASRs/RO</span>
            <span class="val">${fmtNumLocal(asrPer,2)}</span>
          </div>

          <div class="cell">
            <span class="lbl">Total Sold</span>
            <span class="val">${fmtIntLocal(sold_total)}</span>
            <div class="sub" style="margin-top:2px;opacity:.85">ASRs: ${fmtIntLocal(sold_asrs)} · Adv: ${fmtIntLocal(advisor_sold)}</div>
          </div>

          <div class="cell">
            <span class="lbl">Sold/RO</span>
            <span class="val">${fmtPctLocal(sold_ro)}</span>
            <div class="sub" style="margin-top:2px;opacity:.85">Sold ASRs%: ${fmtPctLocal(sold_asrs_pct)}</div>
          </div>
        </div>
      </div>
    `;
  ...
  app.innerHTML = `<div class="pageAdvisorDash" style="display:grid;gap:12px">${header}<div class="panel"><div class="phead"><div class="h2">Advisors</div><div class="sub">Sorted by Total Sold/RO</div></div><div class="list">${rows}</div></div></div>`;
}
